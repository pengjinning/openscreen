...
```powershell
pnpm run test:cursor-native:win
```
...
```powershell
pnpm run capture:openscreen-preview
```
...
By default it uses the latest `cursor-recording-data.json` generated by `pnpm run test:cursor-native:win`. To force a specific sidecar:
...
pnpm run test:wgc-helper:win -- --capture-cursor
